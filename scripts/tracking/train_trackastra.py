"""Script to train the TrackAstra model."""
...
def _git_commit():
    """Get the current git commit hash for TrackAstra.

    Returns the git commit hash of the current repository
    if it exists, otherwise None (for debugging purposes).
    """
...
def log_tracking_metrics(model, _data, causal_norm: str, delta: int):
    """Log the current tracking metrics."""
...
# define the LightningModule that contains the TrackingTransformer
# (to separate torch and lightning) this contains all the training/loss logic
class WrappedLightningModule(pl.LightningModule):
    """A wrapper around the TrackingTransformer to use it with PyTorch Lightning."""
    ...
    def _common_step(self, batch, eps=None):
        if eps is None:
        ...
        if self.causal_norm != "none":
            # TODO speedup: I could softmax only the part of the matrix
            #  (upper triangular) that is not masked out
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == self.batch_val_tb_idx:
            self.batch_val_tb = {"batch": batch, "out": out}

        return loss

    def on_validation_epoch_end(self):
        """Callback from when the validation epoch ends."""
...
class ExampleImages(pl.pytorch.callbacks.Callback):
    """Logger callback to log example images to tensorboard."""
    ...
    def on_train_start(self, trainer, pl_module):
        """Callback from when the training starts."""
        start = default_timer()
...
class MyModelCheckpoint(pl.pytorch.callbacks.Callback):
    """A model checkpoint that saves the model using TrackingTransformer.save()."""
    ...
    def on_fit_start(self, trainer, pl_module):
        """Callback from when the fit() routine is started."""
    ...
    def on_validation_end(self, trainer, pl_module):
        """Callback from when the validation step ends."""
...
def create_run_name(args):
    """Create a name for this training run."""
...
def cache_class(cachedir=None):
    ...
    def hash_args_kwargs(*args, **kwargs):
        """Make a hash of keyword arguments."""
...
def find_val_batch(loader_val, n_gpus):
    """Find the validation batch with the most divisions for visualization."""
    # find the val batch with most divisions for visualization, which runs on GPU 0
    batches_val = tuple(
...
def train(args):
    ...
    datamodule = None
    # Sampler gets wrapped with distributed sampler,
    # which cannot sample with replacement
    if args.distributed:
...
def parse_train_args():
    ...
    if not {a.split("=")[0].strip("-") for a in unknown_args}.issubset(
        set(allowed_unknown)
    ):
    ...
    if args.distributed and hasattr(sys, "ps1"):
        raise ValueError(
            "Distributed training does not work in interactive mode."
